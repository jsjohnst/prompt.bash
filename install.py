...
"""
In order to install prompt.bash a few lines have to be appended to those files sourced every time
...
FUNCTION_CMD = 'function _update_ps1() {{ export PS1="$({}/prompt.py $?)"; }}'
...
class ConfigFile(object):
    ...
    def _is_already_setup(self):
        ...
        # So the file does not contain `FUNCTION_CMD`.
        print('prompt.bash already installed in {}'.format(self.path))
        return True
...
if __name__ == '__main__':
    ...
    if to_be_installed:
        ...
        print('\nWe are about to install prompt.bash in {}'.format(text))
